...
const ISSUE_AREAS = [
  { value: "all", label: "Supported sectors (labor, housing, fiscal)" },
...
export default function App() {
  ...
  // Demo is opt-in at build time. The desktop bundle always calls its bundled backend.
  ...
  const demoResponse = useMemo(
    () => ({
      ...
      data_mode: "fixture",
      data_notice: "OFFLINE DEMO OUTPUT: these hard-coded sample values are illustrative only and were not generated by the backend.",
    }),
    ...
    <div className="page">
      <header className="hero">
        <div className="hero-text">
          ...
          <h1>Traceable indicators and transparent policy prioritization.</h1>
          <p className="subhead">
            Supports labor, housing, and fiscal indicators. Results always identify live versus offline fixture data.
          </p>
        </div>
      ...
      <section className="panel">
        ...
        <div className="grid">
          ...
          <div className="mode-note">
            <strong>{demoMode ? "OFFLINE DEMO MODE" : "Backend mode"}</strong>
            <span>{demoMode ? "Hard-coded sample output only. It is not generated analysis." : "Uses the bundled backend. Each result reports whether its data is live or an offline fixture."}</span>
          </div>
        </div>
      ...
      {advice && (
        <section className="results">
          <div className={`data-notice ${advice.data_mode || "unknown"}`}>
            <strong>Data source: {(advice.data_mode || "unknown").toUpperCase()}</strong>
            <span>{advice.data_notice || "Review citations before relying on this result."}</span>
          </div>
          <div className="card">
          ...
          <div className="card">
            ...
            <p className="muted">Forecasts are withheld when the history is too short or the requested horizon is too long.</p>
            ...
            <div className="options">
              {advice.options.map((option) => (
                <div key={option.title} className="option">
                  ...
                  <p className="notes">{option.scoring_note}</p>
            ...
            <ul>
              {advice.citations.map((citation) => (
                <li key={citation.citation_id}>
                  <strong>{citation.citation_id}</strong>: {citation.url} (retrieved {citation.retrieval_date}; {citation.data_mode || "unknown"} data)
                </li>
              ))}
            </ul>
          </div>
